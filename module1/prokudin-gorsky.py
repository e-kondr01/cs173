...
def find_optimal_shift(img1, img2, px_shift: int = 15):
    ...
    max_correlation_x = 0
    for x in range(-px_shift, px_shift + 1):
        shifted_img2 = roll(img2, x, axis=0)
        correlation = (img1 * shifted_img2).sum()
        if correlation > max_correlation_x:
            max_correlation_x = correlation
            optimal_shift_x = x
    max_correlation_y = 0
    for y in range(-px_shift, px_shift + 1):
        shifted_img2 = roll(img2, y, axis=1)
        correlation = (img1 * shifted_img2).sum()
        if correlation > max_correlation_y:
            max_correlation_y = correlation
            optimal_shift_y = y
    print(optimal_shift_x, optimal_shift_y)
...
def align(img, g_coord):
    ...
    imsave('g.png', g)
    imsave('b.png', b)
    no_shift = dstack((r, g, b))
    imsave('no_shift.png', no_shift)
